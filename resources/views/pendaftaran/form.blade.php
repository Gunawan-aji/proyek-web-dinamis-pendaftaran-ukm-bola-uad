...
<head>
    ...
    {{-- PERBAIKAN: Menggunakan asset() untuk CSS --}}
...
<body class="register-body">
    <div class="register-wrapper">
        {{-- PERBAIKAN: Menggunakan route() untuk Kembali ke Beranda --}}
        ...
        <div class="register-container">
            ...
            {{-- PERBAIKAN PENTING #1: Ganti action ke route('account.store') --}}
            {{-- PERBAIKAN PENTING #2: Hapus enctype="multipart/form-data" karena tidak ada file upload --}}
            <form action="{{ route('account.store') }}" method="POST" class="register-form">
                ...
                <div class="form-grid">
                    <div class="form-group">
                        ...
                        @error('nama_lengkap')<span class="error-message" style="color: red;">{{ $message }}</span>@enderror
                    </div>

                    <div class="form-group">
                        <label for="password">Password (Minimal 8 Karakter)</label>
                        <input type="password" id="password_input" name="password" placeholder="Minimal 8 karakter" required>
                        @error('password')<span class="error-message" style="color: red;">{{ $message }}</span>@enderror
                    </div>

                    <div class="form-group">
                        <label for="nim">NIM</label>
                        <input type="text" id="nim" name="nim" placeholder="Masukkan Nim" value="{{ old('nim') }}" required>
                    ...
                    <div class="form-group">
                        ...
                        <input type="email" id="email_kampus" name="email_kampus" placeholder="Masukkan Email Kampus" value="{{ old('email_kampus') }}" required>
                        @error('email_kampus')<span class="error-message" style="color: red;">{{ $message }}</span>@enderror
                    </div>
